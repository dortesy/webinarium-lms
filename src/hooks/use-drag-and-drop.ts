...
export const useDragAndDrop = (
  ...
  setSections: (sections: SectionWithLessons[]) => void,
  setError: (error: string | undefined) => void,
  updateSectionsOrder: (sections: SectionWithLessons[]) => void,
  pathname: string,
) => {
  const handleDragEnd = useCallback(
    (event: any) => {
      ...
      EditSectionPosition(active.id as string, newPosition, pathname)
        .then((data) => {
          if ('error' in data) {
            setError(data.error);
          } else {
            setTimeout(() => {
              updateSectionsOrder(data.sections);
            }, 500);
          }
        })
        .catch((error) => {
...
const calculateNewPosition = (
  ...
  if (overIndex > activeIndex) {
    ...
    return nextSection
      ? (targetSection.position + nextSection.position) / 2
      : targetSection.position + 1;
  } else {
    const previousSection = sections[overIndex - 1];
    const targetSection = sections[overIndex];
    return previousSection
      ? (previousSection.position + targetSection.position) / 2
      : targetSection.position / 2;
  }
};
